...
pub struct Triangle {
    pub a_x: i32, pub a_y: i32,  // Vertex A coordinates
    pub b_x: i32, pub b_y: i32,  // Vertex B coordinates
    pub c_x: i32, pub c_y: i32,  // Vertex C coordinates
...
fn orient(ax: i32, ay: i32, bx: i32, by: i32, cx: i32, cy: i32) -> i32 {
    ...
    if det > 0 { 1 } else if det < 0 { -1 } else { 0 }
}
...
pub fn encode_triangle(triangle: &Triangle) -> [u8; TOTAL_BYTES] {
    ...
    let mut a_x = triangle.a_x; let mut a_y = triangle.a_y;
    let mut b_x = triangle.b_x; let mut b_y = triangle.b_y;
    let mut c_x = triangle.c_x; let mut c_y = triangle.c_y;
    let mut ab = triangle.ab; let mut bc = triangle.bc; let mut ca = triangle.ca;
    ...
    if b_x < a_x || c_x < a_x {
        if b_x < c_x {
            ...
            let temp_x = a_x; let temp_y = a_y; let temp_bool = ab;
            a_x = b_x; a_y = b_y; ab = bc;
            b_x = c_x; b_y = c_y; bc = ca;
            c_x = temp_x; c_y = temp_y; ca = temp_bool;
        } else {
            // Rotate: a -> c -> b -> a (C becomes new A)
            let temp_x = a_x; let temp_y = a_y; let temp_bool = ab;
            a_x = c_x; a_y = c_y; ab = ca;
            c_x = b_x; c_y = b_y; ca = bc;
            b_x = temp_x; b_y = temp_y; bc = temp_bool;
        }
    ...
    if orient(a_x, a_y, b_x, b_y, c_x, c_y) == -1 {
        ...
        let temp_x = b_x; let temp_y = b_y; let temp_bool = ab;
...
pub fn decode_triangle(bytes: &[u8; TOTAL_BYTES]) -> Triangle {
    ...
    let (a_x, a_y, b_x, b_y, c_x, c_y) = match t_code {
        ...
        _ => panic!("Could not decode the provided triangle - invalid encoding pattern: {}", t_code),
    };
    ...
    assert!(orient(a_x, a_y, b_x, b_y, c_x, c_y) >= 0,
...
fn main() {
    println!("Triangle Encoding/Decoding Example");
    println!("==================================");

    // Create a test triangle
    let original_triangle = Triangle::new(
        100, 200,  // Point A (100, 200)
        300, 150,  // Point B (300, 150)
        250, 400,  // Point C (250, 400)
        true,      // Edge AB is from original shape
        false,     // Edge BC is not from original shape
        true,      // Edge CA is from original shape
    );

    println!("Original Triangle:");
    println!("  A: ({}, {})", original_triangle.a_x, original_triangle.a_y);
    println!("  B: ({}, {})", original_triangle.b_x, original_triangle.b_y);
    println!("  C: ({}, {})", original_triangle.c_x, original_triangle.c_y);
    println!("  Edges from shape: AB={}, BC={}, CA={}",
             original_triangle.ab, original_triangle.bc, original_triangle.ca);

    // Encode the triangle
    let encoded_bytes = encode_triangle(&original_triangle);
    println!("\nEncoded as 7D point (28 bytes):");
    for (i, chunk) in encoded_bytes.chunks(4).enumerate() {
        let value = sortable_bytes_to_int(&encoded_bytes, i * 4);
        println!("  Dimension {}: {} (bytes: {:02x} {:02x} {:02x} {:02x})",
                 i, value, chunk[0], chunk[1], chunk[2], chunk[3]);
    }

    // Decode the triangle
    let decoded_triangle = decode_triangle(&encoded_bytes);
    println!("\nDecoded Triangle:");
    println!("  A: ({}, {})", decoded_triangle.a_x, decoded_triangle.a_y);
    println!("  B: ({}, {})", decoded_triangle.b_x, decoded_triangle.b_y);
    println!("  C: ({}, {})", decoded_triangle.c_x, decoded_triangle.c_y);
    println!("  Edges from shape: AB={}, BC={}, CA={}",
             decoded_triangle.ab, decoded_triangle.bc, decoded_triangle.ca);

    // Verify round-trip encoding
    let matches = original_triangle == decoded_triangle;
    println!("\nRound-trip encoding successful: {}", matches);

    if !matches {
        println!("WARNING: Decoded triangle does not match original!");
        println!("This may be due to triangle rotation during encoding for canonical form.");
    }

    // Show the 4D bounding box (first 4 dimensions)
    let min_y = sortable_bytes_to_int(&encoded_bytes, 0 * BYTES);
    let min_x = sortable_bytes_to_int(&encoded_bytes, 1 * BYTES);
    let max_y = sortable_bytes_to_int(&encoded_bytes, 2 * BYTES);
    let max_x = sortable_bytes_to_int(&encoded_bytes, 3 * BYTES);

    println!("\n4D Bounding Box (first 4 dimensions for BKD tree indexing):");
    println!("  minY: {}", min_y);
    println!("  minX: {}", min_x);
    println!("  maxY: {}", max_y);
    println!("  maxX: {}", max_x);
    println!("  2D Bounding Rectangle: ({}, {}) to ({}, {})", min_x, min_y, max_x, max_y);
}
...
mod tests {
    ...
    #[test]
    fn test_triangle_encoding_roundtrip() {
        let triangle = Triangle::new(10, 20, 30, 40, 50, 60, true, false, true);
        let encoded = encode_triangle(&triangle);
        let decoded = decode_triangle(&encoded);

        // Note: Due to canonical form rotation, coordinates may be reordered
        // but the triangle should represent the same geometric shape
        assert_eq!(encoded.len(), TOTAL_BYTES);
    }

    #[test]
    fn test_sortable_bytes_conversion() {
        let test_values = vec![0, 1, -1, 1000, -1000, i32::MAX, i32::MIN];

        for value in test_values {
            let mut bytes = [0u8; 4];
            int_to_sortable_bytes(value, &mut bytes, 0);
            let decoded = sortable_bytes_to_int(&bytes, 0);
    ...
    fn test_orientation() {
        // CCW triangle
        assert_eq!(orient(0, 0, 1, 0, 0, 1), 1);
        // CW triangle
        assert_eq!(orient(0, 0, 0, 1, 1, 0), -1);
        // Collinear points
