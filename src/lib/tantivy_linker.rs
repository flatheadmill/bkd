...
use crate::storage::{NodeLinker, NodeRef};
...
pub struct TantivyLinker<T> {
    ...
    nodes: HashMap<NodeRef, Node<BoundingBox, T>>,
    file_prefix: String,
}
...
impl<T: Clone> TantivyLinker<T> {
    ...
    fn get_node_filename(&self, node_ref: NodeRef) -> String {
...
impl<T: Clone + serde::Serialize + serde::de::DeserializeOwned> NodeLinker<BoundingBox, T>
    for TantivyLinker<T>
{
    fn get_point(&self, node_ref: NodeRef) -> &BoundingBox {
        &self.nodes.get(&node_ref).unwrap().point
    }

    fn get_data(&self, node_ref: NodeRef) -> &T {
        &self.nodes.get(&node_ref).unwrap().data
    }

    fn get_left(&self, node_ref: NodeRef) -> Option<NodeRef> {
        self.nodes.get(&node_ref)?.left
    }

    fn get_right(&self, node_ref: NodeRef) -> Option<NodeRef> {
        self.nodes.get(&node_ref)?.right
    }

    fn link_left(&mut self, parent_ref: NodeRef, child_ref: NodeRef) {
        if let Some(parent) = self.nodes.get_mut(&parent_ref) {
            parent.left = Some(child_ref);
        }
    }

    fn link_right(&mut self, parent_ref: NodeRef, child_ref: NodeRef) {
        if let Some(parent) = self.nodes.get_mut(&parent_ref) {
